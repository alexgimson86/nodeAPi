    ...
    if (event.which === 13 && $(this).val() !== "") {
        ...
        $.ajax({
            ...
            .done((returnedTodo) => {
                ...
                $('ul').append(
                    `<li data-id="${returnedTodo[0].id}" class="${isComplete}"><span>X</span>${returnedTodo[0].description}</li>`
                );
                $('input').val('');
            })
            .fail((error) => {
...
$('ul').on('click', 'li', function(){
    var url = `${link}/username/${name}/${$(this).data('id')};`
    let that = $(this);
    $.ajax({
        url: url,
        method: 'PUT'
...
$('ul').on('click', 'span', function (event) {
    ...
    var url = `${link}/username/${name}/todo/${thisId}`;
    $.ajax({
        url: url,
        method: 'DELETE',
    }) .done(function(){
